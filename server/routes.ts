...
function makeRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("timeout", () => {
      req.destroy();
      reject(new Error("Request timed out - check the speaker IP address"));
    });
    req.end();
  });
}
...
export async function registerRoutes(
  ...
  app.post("/api/speaker/status", async (req, res) => {
    ...
    if (!isPrivateIP(ipAddress)) {
      return res.status(403).json({ error: "Only private network IP addresses are allowed" });
    }
    ...
    try {
      ...
      if (volumeData.status === "rejected" && muteData.status === "rejected") {
        return res.status(502).json({
          error: volumeData.reason?.message || "Cannot reach speaker",
          connected: false,
        });
      }
